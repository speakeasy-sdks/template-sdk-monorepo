...
import { GetItemReceiptRequest } from "@speakeasy-sdks/accounting/dist/sdk/models/operations";
...
const itemReceiptId: string = "string";

  const res = await sdk.itemReceipts.get(companyId, connectionId, itemReceiptId);
...
(async() => {
  ...
  const res = await sdk.itemReceipts.list({
