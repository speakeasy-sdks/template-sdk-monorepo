/*
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

import * as shared from "../shared";
import * as z from "zod";

export type DownloadInvoiceAttachmentRequest = {
    /**
     * Unique identifier for an attachment.
     */
    attachmentId: string;
    /**
     * Unique identifier for a company.
     */
    companyId: string;
    /**
     * Unique identifier for a connection.
     */
    connectionId: string;
    /**
     * Unique identifier for an invoice.
     */
    invoiceId: string;
};

export type DownloadInvoiceAttachmentResponse = {
    /**
     * HTTP response content type for this operation
     */
    contentType: string;
    /**
     * Success
     */
    data?: ReadableStream<Uint8Array> | undefined;
    /**
     * Your API request was not properly authorized.
     */
    errorMessage?: shared.ErrorMessage | undefined;
    /**
     * HTTP response status code for this operation
     */
    statusCode: number;
    /**
     * Raw HTTP response; suitable for custom response parsing
     */
    rawResponse: Response;
};

/** @internal */
export namespace DownloadInvoiceAttachmentRequest$ {
    export type Inbound = {
        attachmentId: string;
        companyId: string;
        connectionId: string;
        invoiceId: string;
    };

    export const inboundSchema: z.ZodType<DownloadInvoiceAttachmentRequest, z.ZodTypeDef, Inbound> =
        z
            .object({
                attachmentId: z.string(),
                companyId: z.string(),
                connectionId: z.string(),
                invoiceId: z.string(),
            })
            .transform((v) => {
                return {
                    attachmentId: v.attachmentId,
                    companyId: v.companyId,
                    connectionId: v.connectionId,
                    invoiceId: v.invoiceId,
                };
            });

    export type Outbound = {
        attachmentId: string;
        companyId: string;
        connectionId: string;
        invoiceId: string;
    };

    export const outboundSchema: z.ZodType<
        Outbound,
        z.ZodTypeDef,
        DownloadInvoiceAttachmentRequest
    > = z
        .object({
            attachmentId: z.string(),
            companyId: z.string(),
            connectionId: z.string(),
            invoiceId: z.string(),
        })
        .transform((v) => {
            return {
                attachmentId: v.attachmentId,
                companyId: v.companyId,
                connectionId: v.connectionId,
                invoiceId: v.invoiceId,
            };
        });
}

/** @internal */
export namespace DownloadInvoiceAttachmentResponse$ {
    export type Inbound = {
        ContentType: string;
        Data?: ReadableStream<Uint8Array> | undefined;
        ErrorMessage?: shared.ErrorMessage$.Inbound | undefined;
        StatusCode: number;
        RawResponse: Response;
    };

    export const inboundSchema: z.ZodType<
        DownloadInvoiceAttachmentResponse,
        z.ZodTypeDef,
        Inbound
    > = z
        .object({
            ContentType: z.string(),
            Data: z.instanceof(ReadableStream<Uint8Array>).optional(),
            ErrorMessage: shared.ErrorMessage$.inboundSchema.optional(),
            StatusCode: z.number().int(),
            RawResponse: z.instanceof(Response),
        })
        .transform((v) => {
            return {
                contentType: v.ContentType,
                ...(v.Data === undefined ? null : { data: v.Data }),
                ...(v.ErrorMessage === undefined ? null : { errorMessage: v.ErrorMessage }),
                statusCode: v.StatusCode,
                rawResponse: v.RawResponse,
            };
        });

    export type Outbound = {
        ContentType: string;
        Data?: ReadableStream<Uint8Array> | undefined;
        ErrorMessage?: shared.ErrorMessage$.Outbound | undefined;
        StatusCode: number;
        RawResponse: never;
    };

    export const outboundSchema: z.ZodType<
        Outbound,
        z.ZodTypeDef,
        DownloadInvoiceAttachmentResponse
    > = z
        .object({
            contentType: z.string(),
            data: z.instanceof(ReadableStream<Uint8Array>).optional(),
            errorMessage: shared.ErrorMessage$.outboundSchema.optional(),
            statusCode: z.number().int(),
            rawResponse: z.instanceof(Response).transform(() => {
                throw new Error("Response cannot be serialized");
            }),
        })
        .transform((v) => {
            return {
                ContentType: v.contentType,
                ...(v.data === undefined ? null : { Data: v.data }),
                ...(v.errorMessage === undefined ? null : { ErrorMessage: v.errorMessage }),
                StatusCode: v.statusCode,
                RawResponse: v.rawResponse,
            };
        });
}
