...
async function run() {
  ...
  const result = await sdk.loanWriteback.suppliers.getCreateUpdateModel(companyId, connectionId);
...
```typescript
import { LendingTs } from "@speakeasy-sdks/lending";
import { AccountingAddressType, SupplierStatus } from "@speakeasy-sdks/lending/models/components";
...
async function run() {
  ...
  const result = await sdk.loanWriteback.suppliers.create({
