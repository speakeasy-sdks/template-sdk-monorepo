...
async function run() {
  ...
  const result = await sdk.sales.transactions.list({
...
async function run() {
  ...
  const result = await sdk.sales.transactions.get(companyId, connectionId, transactionId);
