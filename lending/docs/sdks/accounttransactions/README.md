...
async function run() {
  ...
  const result = await sdk.transactions.accountTransactions.list({
...
async function run() {
  ...
  const result = await sdk.transactions.accountTransactions.get(companyId, connectionId, accountTransactionId);
