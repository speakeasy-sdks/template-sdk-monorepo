...
async function run() {
  ...
  const result = await sdk.banking.transactionCategories.list({
...
async function run() {
  ...
  const transactionCategoryId = "<value>";
  
  const result = await sdk.banking.transactionCategories.get(companyId, connectionId, transactionCategoryId);
