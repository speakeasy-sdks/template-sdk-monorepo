...
async function run() {
  ...
  const result = await sdk.loanWriteback.directCosts.getCreateModel(companyId, connectionId);
...
```typescript
import { LendingTs } from "@speakeasy-sdks/lending";
import { TrackingRecordRefDataType } from "@speakeasy-sdks/lending/models/components";
...
async function run() {
  ...
  const result = await sdk.loanWriteback.directCosts.create({
    ...
    directCostPrototype: {
      ...
      lineItems: [
        {
          unitAmount: new Decimal("6384.24"),
      ...
      subTotal: new Decimal("9967.06"),
      taxAmount: new Decimal("7964.74"),
      totalAmount: new Decimal("3768.44"),
      supplementalData: {
        content: {
          "key": {
            "key": "<value>",
