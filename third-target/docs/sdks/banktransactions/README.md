...
async function run() {
  ...
  const result = await sdk.loanWriteback.bankTransactions.getCreateModel(companyId, connectionId, accountId);
...
async function run() {
  ...
  const result = await sdk.loanWriteback.bankTransactions.create({
